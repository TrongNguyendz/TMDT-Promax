...
exports.createOrder = async (req, res) => {
    ...
    try {
        ...
        if (!final_user_id || !items || items.length === 0 || !final_shipping_address) {
            return res.status(400).json({ success: false, message: 'Dữ liệu không hợp lệ: Thiếu thông tin bắt buộc' });
        }
        ...
        const calculated_total = Number(final_amount || total_amount || totalAmount || amount || 0);
        ...
        for (const item of items) {
            try {
                ...
                if (newStock < 0) {
                    throw new Error(`Sản phẩm ${productData.name} đã hết hàng (Chỉ còn ${currentStock})`);
                }
                ...
                if (createdOrderId) {
                    ...
                    console.log(`⚠️ Đã Rollback (Xóa) đơn hàng #${createdOrderId} do lỗi kho`);
                }
        ...
        try {
            ...
            const customerName = final_shipping_address?.full_name || final_customer_name || 'Khách lẻ';
            const customerPhone = final_shipping_address?.phone || final_customer_phone || '';
            ...
            const customerAddress = fullAddressParts.join(', ') || 'Không có thông tin địa chỉ';
            ...
            const invoiceItems = items.map(item => {
                ...
                const description = `${item.product_name || 'Sản phẩm'} ${variant ? `(${variant})` : ''}`.trim();
            ...
            axios.post('http://localhost:3005/api/notifications', notificationPayload)
                ...
                .catch(err => {
                    console.error('⚠️ Lỗi gửi yêu cầu hóa đơn (đơn hàng vẫn tạo thành công):', 
                        err.response?.data || err.message);
                });

        } catch (notifyError) {
            console.error('⚠️ Lỗi chuẩn bị dữ liệu hóa đơn (không ảnh hưởng đến đơn hàng):', notifyError.message);
        }
        // Trả về kết quả thành công
        res.status(201).json({ 
            success: true, 
            message: 'đặt hàng thành công , vui lòng check hóa đơn được gửi đến gmail', 
            data: orderResult 
        });
        ...
        if (order.items && order.items.length > 0) {
            for (const item of order.items) {
                try {
                    ...
                    if (product) {
                        const newStock = parseInt(product.stock_quantity || 0) + parseInt(item.quantity);
...
exports.deleteOrder = async (req, res) => {
    try {
        ...
        if (!success) return res.status(404).json({ success: false, message: 'Không tìm thấy đơn' });
...
exports.getReportStats = async (req, res) => {
    try {
        ...
        const totalOrdersForCalc = filledStatus.reduce((sum, item) => sum + item.count, 0) || 1;
        ...
        // --- 3. TRẢ VỀ KẾT QUẢ ---
        const avgOrderValue = stats.kpi.total_orders > 0 
