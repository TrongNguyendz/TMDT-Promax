...
<div class="absolute inset-0 flex items-center justify-center gap-4 bg-black/5 opacity-0 backdrop-blur-[2px] transition-all duration-300 group-hover:opacity-100">
        ...
        <div class="p-5">
          <div class="flex items-center justify-between">
            ...
            <span class="text-[9px] font-mono font-bold text-gray-400 uppercase tracking-tighter italic">ID: {{ b.id }}</span>
          </div>
      ...
      <div class="relative w-full max-w-2xl overflow-hidden rounded-[2.5rem] bg-white shadow-2xl dark:bg-gray-950 border border-white/10">
        ...
        <form @submit.prevent="saveBanner" class="p-8 space-y-6 max-h-[75vh] overflow-y-auto custom-scrollbar">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-2">
              <label class="ml-1 text-[10px] font-black uppercase tracking-widest text-gray-400">Tiêu đề Banner *</label>
<input v-model="formData.title" type="text" class="w-full rounded-2xl border-gray-100 bg-gray-50 px-5 py-3.5 text-sm font-bold focus:border-black focus:ring-0 dark:border-gray-800 dark:bg-gray-900 dark:text-white" required />
            </div>
            <div class="space-y-2">
              <label class="ml-1 text-[10px] font-black uppercase tracking-widest text-gray-400">Đường dẫn liên kết</label>
              <input v-model="formData.link" type="url" placeholder="https://..." class="w-full rounded-2xl border-gray-100 bg-gray-50 px-5 py-3.5 text-sm dark:border-gray-800 dark:bg-gray-900 dark:text-white" />
            </div>
          </div>
          ...
          <div class="space-y-2">
            ...
            <textarea v-model="formData.description" rows="2" class="w-full rounded-2xl border-gray-100 bg-gray-50 px-5 py-3.5 text-sm dark:border-gray-800 dark:bg-gray-900 dark:text-white"></textarea>
          </div>
    ...
    <div class="flex items-center justify-between mb-8 border-b border-gray-100 pb-4 dark:border-gray-800">
      ...
      <div class="flex items-center gap-6">
        <label class="flex cursor-pointer items-center gap-2 group">
          <input type="radio" v-model="formData.type" value="image" class="h-4 w-4 border-gray-300 text-black focus:ring-black dark:bg-gray-800" />
          <span class="text-[10px] font-bold uppercase tracking-widest text-gray-400 group-hover:text-black dark:group-hover:text-white transition-colors">🖼️ Image</span>
        </label>
        <label class="flex cursor-pointer items-center gap-2 group">
          <input type="radio" v-model="formData.type" value="video" class="h-4 w-4 border-gray-300 text-black focus:ring-black dark:bg-gray-800" />
          <span class="text-[10px] font-bold uppercase tracking-widest text-gray-400 group-hover:text-black dark:group-hover:text-white transition-colors">🎬 Video</span>
        </label>
      </div>
    </div>
...
<div class="relative group aspect-video w-full overflow-hidden rounded-[1.5rem] bg-white dark:bg-gray-800 border-2 border-dashed border-gray-200 dark:border-gray-700 transition-all hover:border-black dark:hover:border-white flex items-center justify-center shadow-sm">
          <input 
            type="file" 
      ...
      <div class="space-y-3">
        ...
        <div class="relative aspect-video w-full overflow-hidden rounded-[1.5rem] bg-gray-200 dark:bg-black shadow-inner border border-gray-100 dark:border-gray-800">
          <!-- Image Preview -->
          <img 
            v-if="formData.media && formData.type === 'image'" 
            :src="formData.media" 
            class="h-full w-full object-cover animate-in fade-in zoom-in-95 duration-500" 
          />
          <!-- Video Preview -->
          <video 
            v-else-if="formData.media && formData.type === 'video'" 
            :src="formData.media" 
            controls 
...
<div class="flex gap-4 pt-6">
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 z-[80] flex items-center justify-center p-4 backdrop-blur-sm">
      <div class="w-full max-w-sm rounded-[2.5rem] bg-white p-10 text-center shadow-2xl dark:bg-gray-950 border border-gray-100 dark:border-gray-800">
        ...
        <p class="mt-3 text-sm font-medium text-gray-500 leading-relaxed italic">Hành động này sẽ xóa vĩnh viễn <span class="text-gray-900 dark:text-white font-black underline">"{{ bannerToDelete?.title }}"</span> khỏi hệ thống.</p>
