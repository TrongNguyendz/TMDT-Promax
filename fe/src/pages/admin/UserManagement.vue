      ...
      <div class="overflow-x-auto">
        <table class="min-w-full text-sm">
          ...
          <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
            <tr v-for="u in users" :key="u.id" class="group transition-colors hover:bg-gray-50/50 dark:hover:bg-gray-800/30">
...
<!-- User Name: Làm đậm để thay thế cho ảnh làm điểm nhấn -->
              ...
              <td class="px-8 py-5 text-right">
                <div class="flex items-center justify-end gap-3 opacity-0 transition-all duration-300 group-hover:opacity-100 translate-x-2 group-hover:translate-x-0">
                  <button 
                    v-if="u.role !== 'admin'"
    ...
    <!-- 4. Empty State -->
<div v-if="!loading && users.length === 0" class="flex flex-col items-center justify-center py-32 text-center rounded-[2.5rem] bg-gray-50/50 dark:bg-gray-900/50 border-2 border-dashed border-gray-100 dark:border-gray-800">
      <h3 class="text-xl font-black text-gray-300 dark:text-gray-700 uppercase italic tracking-widest">Không có dữ liệu</h3>
    </div>
  </div>
</template>
