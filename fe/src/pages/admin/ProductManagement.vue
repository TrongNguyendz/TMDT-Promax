<template>
    ...
    <div class="mb-8 flex items-end justify-between border-b border-gray-100 pb-6 dark:border-gray-800">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">
                Danh mục kho hàng và điều phối sản phẩm hệ thống
            </p>
        </div>
        <button
            class="group flex items-center gap-2 rounded-xl bg-gray-900 px-6 py-3 text-sm font-bold text-white shadow-lg transition-all hover:bg-black hover:shadow-xl active:scale-95 dark:bg-white dark:text-black dark:hover:bg-gray-200"
            @click="openCreate">
    ...
    <div
        ...
        <div class="flex-1 min-w-[280px]">
            ...
            <div class="relative">
                ...
                <input v-model="filters.q" type="text" placeholder="Tên sản phẩm, SKU..."
                    class="w-full rounded-2xl border border-gray-100 bg-gray-50/50 py-3 pl-11 pr-4 text-sm transition-all focus:border-black focus:ring-0 dark:border-gray-800 dark:bg-gray-950 dark:text-white dark:focus:border-white"
        ...
        <div class="w-full md:w-64">
            <label class="mb-2 block text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 ml-1">Danh
                mục</label>
        ...
        <!-- Filter Button: Nằm cùng hàng với các input -->
        <button @click="handleSearch"
            class="flex h-[46px] items-center justify-center rounded-2xl border border-gray-200 bg-white px-8 text-sm font-bold tracking-widest text-gray-900 transition-all hover:bg-gray-50 active:scale-95 dark:border-gray-800 dark:bg-gray-950 dark:text-white dark:hover:bg-gray-800">
            LỌC KẾT QUẢ
        </button>
    </div>
    ...
    <div v-else
        ...
        <table class="min-w-full text-sm">
            <thead class="bg-gray-50 dark:bg-gray-900/50 text-left border-b dark:border-gray-700">
                <tr>
                    <th class="p-4 font-semibold text-gray-600 dark:text-gray-300">
                        Sản phẩm
                    </th>
                    <th class="p-4 font-semibold text-gray-600 dark:text-gray-300">
                        Danh mục
                    ...
                    <th class="p-4 font-semibold text-gray-600 dark:text-gray-300 text-right">
                        Hành động
                    </th>
                </tr>
            </thead>
                ...
                <tr v-for="p in items" :key="p.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                    <td class="p-4">
                        <div class="flex items-center gap-3">
                            <div
                                class="h-12 w-12 rounded-lg border dark:border-gray-600 overflow-hidden bg-gray-100 flex-shrink-0">
                    ...
                    <td class="p-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            ...
                            <!-- Nút Xóa -->
                            <button @click="openDeleteConfirm(p)"
    ...
    <div v-if="store.pagination && store.pagination.pages > 1" class="mt-6 flex justify-center gap-2">
        <button @click="changePage(store.pagination.page - 1)" :disabled="store.pagination.page === 1"
            class="px-3 py-1 border rounded-lg disabled:opacity-50 hover:bg-gray-50">
            Trước
        </button>
        <span
            class="px-4 py-1 bg-blue-50 text-blue-600 dark:bg-blue-900/20 dark:text-blue-400 rounded-lg font-medium border border-blue-100 dark:border-blue-800">{{
                store.pagination.page }}</span>
    ...
    <div v-if="showEditModal"
        class="fixed inset-0 bg-black/60 dark:bg-black/80 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
        <div
            class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
            <div
                class="flex items-center justify-between px-6 py-4 border-b dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800">
                <h2 class="text-xl font-bold text-gray-800 dark:text-white">
                    {{ isCreating ? "Thêm sản phẩm mới" : "Cập nhật sản phẩm" }}
                </h2>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-xs font-medium text-gray-500 mb-1">Tên SP *</label><input
                                        v-model="editFormData.name" required
                                        class="w-full px-3 py-2 rounded-lg border dark:bg-gray-700" />
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-gray-500 mb-1">Mã SKU *</label><input
                                        v-model="editFormData.sku" required
                                        class="w-full px-3 py-2 rounded-lg border dark:bg-gray-700" />
                                </div>
                            </div>
                            <div class="grid grid-cols-3 gap-4">
                                <div>
                                    <label class="block text-xs font-medium text-gray-500 mb-1">Giá *</label><input
                                        v-model="editFormData.price" type="number" required
                                        class="w-full px-3 py-2 rounded-lg border dark:bg-gray-700" />
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-gray-500 mb-1">Kho *</label><input
                                        v-model="editFormData.stock_quantity" type="number" required
                                        class="w-full px-3 py-2 rounded-lg border dark:bg-gray-700" />
                                </div>
                                <div>
                                    ...
                                    <div class="flex gap-1">
                                        <select v-model="editFormData.category_id" required
                                            class="w-full px-3 py-2 rounded-lg border dark:bg-gray-700 text-sm">
                                        ...
                                        <button type="button" @click="showCategoryModal = true"
                                            class="px-2 bg-green-100 text-blue-600 rounded-lg hover:bg-blue-200"
                            ...
                            <div>
                                <label class="block text-xs font-medium text-gray-500 mb-1">Mô tả</label><textarea
                                    v-model="editFormData.description" rows="3"
                                    class="w-full px-3 py-2 rounded-lg border dark:bg-gray-700"></textarea>
                            </div>
                        </div>

                        <!-- Thuộc tính -->
                        <div
                            class="space-y-4 bg-gray-50 dark:bg-gray-700/30 p-4 rounded-xl border dark:border-gray-700">
                            <h3 class="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider mb-2">
                                Phân loại
                            </h3>
                            <div>
                                ...
                                <div class="flex gap-2 mb-3">
                                    <div class="relative group">
                                        <input v-model="newColor.hex" type="color"
                                            class="w-9 h-9 p-0 border-0 rounded cursor-pointer overflow-hidden" />
                                        <div
                                            class="absolute inset-0 rounded border border-gray-300 pointer-events-none">
                                        </div>
                                    </div>
                                    <input v-model="newColor.name" placeholder="Tên màu (VD: Đỏ đô)"
                                        class="flex-1 px-3 py-1.5 text-sm border rounded-lg dark:bg-gray-700 dark:border-gray-600 outline-none"
                                        @keyup.enter="addColor" />
                                    <button type="button" @click="addColor"
                                        class="px-3 bg-gray-800 dark:bg-gray-600 text-white rounded-lg text-sm hover:bg-gray-700">
                                        Thêm
                                    </button>
                                </div>
                                ...
                                <div class="flex flex-wrap gap-2 mb-3 min-h-[30px]">
                                    <div v-for="(color, idx) in editFormData.colors" :key="idx"
                                        class="flex items-center gap-2 bg-white dark:bg-gray-600 px-3 py-1 rounded-full border shadow-sm group">
                                        <div class="w-3 h-3 rounded-full border"
                                            :style="{ backgroundColor: color.hex }"></div>
                                        <span class="text-xs font-medium">{{ color.name }}</span>
                                        <button type="button" @click="removeColor(idx)"
                                            class="text-gray-400 hover:text-red-500 ml-1">
                                            ✕
                                        </button>
                                    </div>
                                    <span v-if="editFormData.colors.length === 0"
                                        class="text-xs text-gray-400 italic py-1">Chưa có màu nào được chọn</span>
                                </div>

                                <!-- Gợi ý màu -->
                                <div class="flex flex-wrap gap-1.5">
                                    <button v-for="p in colorPresets" :key="p.hex" type="button" @click="
                                        () => {
                                            newColor.hex = p.hex;
                                            newColor.name = p.name;
                                        }
                            ...
                            <div>
                                ...
                                <div class="flex flex-wrap gap-2">
                                    <label v-for="size in predefinedSizes" :key="size"
                                        class="cursor-pointer select-none">
                                        <input type="checkbox" :value="size" v-model="editFormData.sizes"
                                            class="hidden peer" />
                                        <span
                    ...
                    <div>
                        <div class="flex justify-between items-end mb-3">
                            ...
                            <span class="text-xs font-medium px-2 py-1 bg-gray-100 dark:bg-gray-700 rounded">{{
                                editFormData.images.length }}/10</span>
                        </div>
                        <div v-if="editFormData.images.length > 0" class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-4">
                            <div v-for="(img, idx) in editFormData.images" :key="idx"
                                class="relative group bg-white dark:bg-gray-700 p-2 rounded-lg border">
                                ...
                                <select v-model="img.selectedColor"
                                    class="w-full text-xs py-1 border rounded dark:bg-gray-600">
                                    <option :value="null">Chung</option>
                                    <option v-for="color in editFormData.colors" :key="color.hex" :value="color.hex">
                                        {{ color.name }}
                                    </option>
                                </select>
                            </div>
                        ...
                        <div
                            class="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl p-4 text-center cursor-pointer relative">
            ...
            <div class="px-6 py-4 border-t dark:border-gray-700 flex justify-end gap-3">
                <button type="button" @click="closeEditModal"
                    class="px-5 py-2.5 bg-green border rounded-lg hover:bg-gray-50">
                    Hủy
                </button>
                <button @click="onSubmit" class="px-6 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700"
                    :disabled="store.loading">
                    {{ store.loading ? "Đang lưu..." : "Lưu" }}
                </button>
            </div>
        </div>
    </div>
    ...
    <div v-if="showCategoryModal" class="fixed inset-0 bg-black/60 flex items-center justify-center z-[60] p-4">
        <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-xl max-w-md w-full">
            ...
            <div class="max-h-60 overflow-y-auto mb-4 border rounded dark:border-gray-700 p-2 space-y-2">
                <div v-if="store.categories.length === 0" class="text-center text-gray-500 text-sm py-4">
                    Chưa có danh mục nào
                </div>
                <div v-for="cat in store.categories" :key="cat.id"
                    class="flex justify-between items-center p-2 bg-gray-50 dark:bg-gray-700 rounded group hover:bg-gray-100 dark:hover:bg-gray-600">
            ...
            <div class="flex gap-2 border-t pt-4 dark:border-gray-700">
                <input v-model="newCategoryName" class="flex-1 px-3 py-2 border rounded-lg dark:bg-gray-700 text-sm"
                    placeholder="Tên danh mục mới..." @keyup.enter="quickAddCategory" />
                <button @click="quickAddCategory"
            ...
            <div class="mt-4 text-right">
                <button @click="showCategoryModal = false" class="text-sm text-gray-500 hover:underline">
                    Đóng
                </button>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-2xl w-full max-h-[85vh] flex flex-col">
            <div class="p-4 border-b dark:border-gray-700 flex justify-between items-center">
                <h3 class="font-bold text-lg dark:text-white">
                    Đánh giá: {{ currentProductForReview?.name }}
                </h3>
                <button @click="showReviewModal = false" class="text-gray-500">
                    ✕
                </button>
            </div>
            <div class="p-4 overflow-y-auto flex-1 space-y-4">
                <div v-if="productReviews.length === 0" class="text-center py-10 text-gray-500">
                    Chưa có đánh giá nào.
                </div>
                <div v-else v-for="review in productReviews" :key="review.id"
                    class="border dark:border-gray-700 rounded-lg p-4 bg-gray-50 dark:bg-gray-700/30">
                    ...
                    <div class="mt-3 pt-3 border-t dark:border-gray-600">
                        ...
                        <div v-else class="flex gap-2 mt-2">
                            <input v-model="review.replyTemp" placeholder="Trả lời..."
                                class="flex-1 px-3 py-1.5 text-sm border rounded dark:bg-gray-600"
                    ...
                    <div class="mt-2 text-right">
                        <button @click="deleteReviewFromModal(review.id)" class="text-xs text-red-500 hover:underline">
                            Xóa
                        </button>
                    </div>
                </div>
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
        <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-xl max-w-sm w-full text-center">
            ...
            <div class="flex gap-3 justify-center mt-4">
                <button @click="showDeleteModal = false" class="px-4 py-2 border rounded hover:bg-gray-50">
                    Hủy
                </button>
...
const colorPresets = [
    ...
    { name: "Rượu vang", hex: "#800000" },
    { name: "Xanh pastel", hex: "#A7C7E7" },
];
...
function edit(p) {
    store.fetchProductById(p.id).then(() => {
        ...
        editFormData.images = data.images
            ? data.images.map((img) => ({
                file: null,
                preview: img.image_url,
                public_id: img.public_id,
                selectedColor: img.color || null,
            }))
            : [];
        ...
        if (data.attributes) {
            data.attributes.forEach((attr) => {
                if (attr.attribute_name === "Màu") {
                    ...
                    if (match)
                        editFormData.colors.push({ name: match[1], hex: match[2] });
                } else if (attr.attribute_name === "Size")
                    editFormData.sizes.push(attr.attribute_value);
            });
        }
        showEditModal.value = true;
    });
...
function handleFileChange(event) {
    ...
    if (editFormData.images.length + files.length > 10)
        return ui.pushToast({ type: "warning", message: "Max 10 ảnh" });
    files.forEach((file) => {
        const reader = new FileReader();
        reader.onload = (e) =>
            editFormData.images.push({
                file: file,
...
function removeImage(i) {
    editFormData.images.splice(i, 1);
}
function addColor() {
    if (newColor.name) {
        editFormData.colors.push({ ...newColor });
        newColor.name = "";
    }
}
function removeColor(i) {
    editFormData.colors.splice(i, 1);
}
...
async function onSubmit() {
    ...
    [
        ...
        "category_id",
        "description",
    ].forEach((k) => formData.append(k, editFormData[k]));
    ...
    editFormData.images.forEach((img) => {
        ...
        if (img.file) formData.append("images", img.file);
            ...
            existingImages.push({ public_id: img.public_id, image_url: img.preview });
    });
    ...
    const attrs = [];
    editFormData.colors.forEach((c) =>
        attrs.push({
            attribute_name: "Màu",
            attribute_value: `${c.name} (${c.hex})`,
        }),
    );
    editFormData.sizes.forEach((s) =>
        attrs.push({ attribute_name: "Size", attribute_value: s }),
    );
    formData.append("attributes", JSON.stringify(attrs));

    const res = isCreating.value
        ? await store.createProduct(formData)
        : await store.updateProduct(editFormData.id, formData);
    if (res.success) {
        ui.pushToast({ type: "success", message: "Thành công" });
    ...
    if (res.success) {
        ...
        if (store.categories.length)
            editFormData.category_id =
                store.categories[store.categories.length - 1].id;
    }
}
...
async function openReviews(product) {
    ...
    try {
        ...
        if (res.data.success)
            productReviews.value = res.data.data.map((r) => ({
                ...r,
                replyTemp: "",
            }));
    } catch (e) {
...
async function submitReply(review) {
    ...
    try {
        const res = await api.put(`/products/reviews/${review.id}/reply`, {
            reply: review.replyTemp,
        });
        if (res.data.success) {
            ui.pushToast({ type: "success", message: "Đã trả lời" });
            review.admin_reply = review.replyTemp;
            review.replyTemp = "";
        }
    } catch (e) {
        ui.pushToast({ type: "error", message: "Lỗi" });
    }
}
