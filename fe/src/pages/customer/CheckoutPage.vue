<template>
  <section class="max-w-6xl mx-auto px-4 py-8">
    <div class="mb-8">
      <h1
        class="text-2xl font-black text-gray-900 dark:text-gray-100 uppercase italic tracking-tighter"
      >
        Thanh toán
      </h1>
    </div>
...
async function createPendingOrderAndGetQR() {
  ...
  try {
    ...
    if (newOrder) {
      ...
      // Lấy QR từ PayOS
      await generatePayOSUrl(
        newOrder.id || newOrder._id,
        newOrder.final_amount,
      );
    }
  } catch (error) {
...
async function generatePayOSUrl(orderId, amount) {
  try {
    ...
    const response = await axios.post(
      "https://tmdt-promax-api-gateway.onrender.com/api/v1/payments/payos/create",
      {
        orderId: orderId,
    ...
    if (confirm("Hủy thanh toán hiện tại để chọn phương thức khác?")) {
      try {
        ...
        await orderStore.cancelOrder(
          successOrder.value.id || successOrder.value._id,
          "Đổi phương thức",
        );
