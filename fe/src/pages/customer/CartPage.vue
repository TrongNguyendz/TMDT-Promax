<template>
  <section class="mx-auto max-w-screen-xl px-4 py-8 lg:py-12">
    ...
    <!-- Giỏ hàng trống -->
    <div v-if="!items.length" class="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-gray-200 py-20 text-center dark:border-gray-800">
      ...
      <RouterLink to="/products" class="mt-4 inline-flex items-center justify-center rounded-xl bg-gray-900 px-8 py-3 text-sm font-bold text-white transition-all hover:bg-black hover:shadow-lg dark:bg-white dark:text-black">
        TIẾP TỤC MUA SẮM
      </RouterLink>
    </div>
      ...
      <div class="lg:col-span-8">
        <ul class="space-y-4">
          ...
          <!-- [END CHANGE] -->
            ...
            <!-- Ảnh sản phẩm -->
            <div class="relative h-24 w-24 shrink-0 overflow-hidden rounded-xl bg-gray-100 dark:bg-gray-900 sm:h-32 sm:w-32">
              <!-- [CHANGE]: Ưu tiên hiển thị ảnh Snapshot (product_image) -->
              <RouterLink :to="`/product/${it.product_id}`">
                <img 
                  :src="it.product_image || it.image || 'https://via.placeholder.com/150'" 
                  :alt="it.product_name" 
...
<RouterLink :to="`/product/${it.product_id}`" class="text-lg font-bold text-gray-900 hover:text-red-600 dark:text-gray-100 dark:hover:text-red-400 line-clamp-2">
                ...
                <div class="mt-2 flex flex-wrap gap-2">
                  <span v-if="it.color" class="inline-flex items-center rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-medium text-gray-600 dark:bg-gray-800 dark:text-gray-400">
                    Màu: {{ displayColor(it.color) }}
                  </span>
                  <span v-if="it.size" class="inline-flex items-center rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-medium text-gray-600 dark:bg-gray-800 dark:text-gray-400">
                    Size: {{ it.size }}
                  </span>
                </div>
                <!-- [END CHANGE] -->
              ...
              <div class="mt-4 flex items-center justify-between">
                <div class="flex items-center overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
                  
                  <!-- [CHANGE] -->
                  <button 
                    @click="updateQty(it, it.quantity - 1)" 
                  ...
                  <input 
                    ...
                    :value="it.quantity" 
                    @change="updateQty(it, $event.target.value)" 
                  />

                  <button 
                    @click="updateQty(it, it.quantity + 1)" 
                    class="flex h-8 w-8 items-center justify-center text-gray-600 hover:bg-gray-200 transition"
                  >+</button>
                  <!-- [END CHANGE] -->
                </div>
...
<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
      ...
      <aside class="lg:col-span-4">
        <div class="sticky top-8 rounded-2xl border border-gray-100 bg-gray-50 p-6 dark:border-gray-800 dark:bg-gray-900/50">
