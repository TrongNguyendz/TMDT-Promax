<template>
  <section v-if="product">
      ...
      <div>
        ...
        <div class="mt-6">
            <div class="flex justify-between items-end mb-2">
                ...
                <button @click="showSizeGuide = true" class="text-sm text-blue-600 hover:text-blue-800 hover:underline flex items-center gap-1 transition-colors cursor-pointer"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg> Hướng dẫn chọn size</button>
            </div>
        ...
        <div class="mt-6 flex gap-3">
          ...
          <button :disabled="!product.inStock" class="flex-1 rounded-lg bg-gray-900 px-6 py-3 font-bold text-white shadow-lg hover:bg-gray-800 hover:shadow-xl active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed transition-all dark:bg-gray-100 dark:text-black dark:hover:bg-gray-200" @click="addToCart">
            THÊM VÀO GIỎ HÀNG
          </button>
    ...
    <div class="mt-16 border-t pt-8 dark:border-gray-800">
      <div class="flex gap-8 border-b dark:border-gray-800 overflow-x-auto">
        ...
        <button @click="activeTab = 'tryon'" class="pb-4 text-lg font-semibold transition-colors relative whitespace-nowrap" :class="activeTab === 'tryon' ? 'text-gray-900 dark:text-white' : 'text-gray-500 hover:text-gray-700'">
      ...
      <div class="py-8 min-h-[300px]">
        ...
        <div v-else-if="activeTab === 'reviews'" class="space-y-8">
             ...
             <div>
                ...
                <div v-if="reviews.length === 0" class="text-center py-10 text-gray-500 border border-dashed rounded-lg">Chưa có đánh giá nào. Hãy là người đầu tiên!</div>
                <div v-else class="space-y-6">
                    <div v-for="review in reviews" :key="review.id" class="border-b pb-6 dark:border-gray-700 last:border-0">
                        <div class="flex justify-between items-start mb-2">
                            <div class="flex gap-3">
                                <div class="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center font-bold text-gray-600">{{ (review.author || 'U').charAt(0).toUpperCase() }}</div>
                                <div>
                                    <h4 class="font-bold text-gray-900 dark:text-white">{{ review.author || 'Người dùng' }}</h4>
                                    <div class="flex text-yellow-400 text-sm mt-0.5"><span v-for="i in 5" :key="i">{{ i <= review.rating ? '★' : '☆' }}</span></div>
                        ...
                        <div v-if="review.admin_reply" class="ml-14 mt-3 bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg border border-blue-100 dark:border-blue-800">
                            ...
                            <p class="text-sm text-gray-800 dark:text-gray-200">{{ review.admin_reply }}</p>
                        </div>
        ...
        <div v-else-if="activeTab === 'tryon'" class="mt-2">
          <br/>
          <h3 class="mb-6 text-xl font-bold text-gray-900 dark:text-white">Phòng thử đồ ảo
            <!-- CHANGE -->
            <button 
          ...
          <transition name="fade" mode="out-in">
            <div v-if="!isTryingOn" key="select" class="grid gap-8 md:grid-cols-2">
              ...
              <div class="h-full flex flex-col">
                <div class="rounded-lg border-2 border-dashed border-gray-300 p-8 text-center dark:border-gray-600 bg-gray-50 dark:bg-gray-800 flex-1 flex flex-col items-center justify-center relative overflow-hidden group">
                  <div v-if="!tryOnImage">
                    ...
                    <button @click="$refs.fileInput.click()" class="inline-block rounded-lg bg-gray-900 px-6 py-2 text-white hover:bg-gray-800 dark:bg-white dark:text-black transition">
                      Chọn ảnh
                    </button>
                  </div>
                  <div v-else class="relative w-full h-full flex items-center justify-center">
                    <img :src="tryOnImage" class="max-h-[400px] max-w-full rounded shadow-md object-contain" alt="Ảnh của bạn" />
                    <button @click="clearTryOnImage" class="absolute top-2 right-2 bg-white/90 text-red-600 p-2 rounded-full hover:bg-white shadow-sm transition">
                      ✕
                    </button>
                  </div>
              ...
              <div class="flex flex-col h-full">
                <div v-if="colors.length === 0" class="p-6 text-center text-gray-500 border rounded-lg bg-gray-50 dark:bg-gray-800">
                  Sản phẩm này chưa có biến thể màu sắc để thử.
                </div>
                <div v-else class="flex-1 flex flex-col">
                  <h4 class="mb-3 font-semibold text-gray-800 dark:text-white">
                    Chọn mẫu thử:
                    <span v-if="tryOnSelectedColor" class="ml-2 text-xs font-normal text-blue-600 bg-blue-50 px-2 py-1 rounded-full border border-blue-100 dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-800">
                  ...
                  <div class="grid grid-cols-3 gap-3 max-h-[400px] overflow-y-auto p-1 custom-scrollbar">
                    ...
                    >
                      <div class="aspect-[3/4] w-full bg-gray-100 dark:bg-gray-900 relative">
                        <img v-if="getColorImage(color.hex)" :src="getColorImage(color.hex)" class="w-full h-full object-cover" :alt="color.name" />
                        ...
                        <div v-if="tryOnSelectedColor?.hex === color.hex" class="absolute top-2 right-2 bg-gray-900 text-white rounded-full p-0.5 shadow-sm dark:bg-white dark:text-black">
                      ...
                      <div class="p-2 text-center border-t dark:border-gray-600">
                        <div class="flex items-center justify-center gap-1.5">
                          <div class="w-3 h-3 rounded-full border border-gray-300 shadow-sm" :style="{ backgroundColor: color.hex }"></div>
                ...
                <div class="mt-6">
                  ...
                  <p v-if="!tryOnImage" class="text-xs text-center mt-2 text-red-500 animate-pulse">* Vui lòng tải ảnh của bạn lên trước</p>
                  <p v-if="tryOnImage && !tryOnSelectedColor" class="text-xs text-center mt-2 text-red-500 animate-pulse">* Vui lòng chọn một màu sản phẩm</p>
                </div>
              </div>
            </div>
            ...
            <div v-else key="result" class="relative">
              ...
              <div class="grid gap-8 md:grid-cols-2 items-center">
                ...
                <div class="text-center">
                  <p class="text-sm font-semibold text-gray-600 dark:text-gray-400 mb-3">Ảnh của bạn</p>
                  <div class="rounded-lg border border-gray-300 dark:border-gray-700 p-4 bg-gray-50 dark:bg-gray-800 inline-block">
                    <img :src="tryOnImage" class="max-h-[500px] max-w-full rounded object-contain shadow-lg" alt="Ảnh gốc" />
                  </div>
                </div>
                ...
                <!-- Kết quả thử đồ -->
                <div class="text-center">
                  <p class="text-sm font-semibold text-gray-600 dark:text-gray-400 mb-3 flex items-center justify-center gap-2">
                    Kết quả thử đồ
                    <span class="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full dark:bg-green-900/30 dark:text-green-300">
                      Màu: {{ tryOnSelectedColor.name }}
                    </span>
                  </p>

                  <div class="rounded-lg border border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 min-h-[500px] flex items-center justify-center overflow-hidden shadow-lg">
                    <!-- Đang xử lý -->
                    <div v-if="isProcessing" class="text-center py-12">
                      <div class="inline-block animate-spin rounded-full h-12 w-12 border-4 border-gray-300 border-t-gray-900 mb-4"></div>
                      <p class="text-lg font-medium text-gray-700 dark:text-gray-300">Đang xử lý thử đồ ảo...</p>
                      <p class="text-sm text-gray-500 mt-2">
                        Đang xử lý thử đồ ảo... ({{ pollAttempts }} / {{ pollMaxAttempts }})
                        <br/>Có thể mất 30–120 giây tùy ảnh
                      </p>
                    </div>
  ...
  <!-- MODAL SIZE GUIDE -->
  <div v-if="showSizeGuide" class="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 backdrop-blur-sm" @click.self="showSizeGuide = false">
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[90vh]">
          <div class="flex justify-between items-center p-4 border-b dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
              <h3 class="text-lg font-bold text-gray-900 dark:text-white">Bảng quy đổi kích cỡ</h3>
              <button @click="showSizeGuide = false" class="text-gray-500 hover:text-gray-800 dark:hover:text-white transition-colors"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg></button>
          </div>
          <div class="flex border-b dark:border-gray-700">
              <button v-for="tab in ['Nam', 'Nữ', 'Trẻ em']" :key="tab" @click="sizeTab = tab" class="flex-1 py-3 text-sm font-bold transition-colors relative" :class="sizeTab === tab ? 'text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700'">{{ tab }}<div v-if="sizeTab === tab" class="absolute bottom-0 left-0 w-full h-0.5 bg-blue-600"></div></button>
          </div>
          <div class="p-6 overflow-y-auto custom-scrollbar text-sm">
              <div v-if="sizeTab === 'Nam'" class="space-y-8 animate-fade-in">
                  <div><h4 class="font-bold text-lg mb-3 dark:text-white border-l-4 border-blue-600 pl-2">Áo Nam</h4><div class="overflow-x-auto border rounded-lg dark:border-gray-700"><table class="w-full text-center"><thead class="bg-gray-100 dark:bg-gray-700 font-bold"><tr><th class="p-2">Size</th><th class="p-2">Chiều cao (cm)</th><th class="p-2">Cân nặng (kg)</th><th class="p-2">Rộng vai</th><th class="p-2">Vòng ngực</th></tr></thead><tbody class="divide-y dark:divide-gray-600"><tr><td class="p-2 font-bold">S</td><td>160-165</td><td>50-54</td><td>41</td><td>82-86</td></tr><tr><td class="p-2 font-bold">M</td><td>160-165</td><td>55-61</td><td>42</td><td>86-90</td></tr><tr><td class="p-2 font-bold">L</td><td>166-172</td><td>62-68</td><td>43.5</td><td>90-94</td></tr><tr><td class="p-2 font-bold">XL</td><td>172-177</td><td>69-75</td><td>45</td><td>94-98</td></tr><tr><td class="p-2 font-bold">2XL</td><td>177-184</td><td>76-84</td><td>46.5</td><td>98-102</td></tr><tr><td class="p-2 font-bold">3XL</td><td>184-192</td><td>85-90</td><td>48</td><td>102-106</td></tr></tbody></table></div></div>
                  <div><h4 class="font-bold text-lg mb-3 dark:text-white border-l-4 border-blue-600 pl-2">Quần Nam</h4><div class="overflow-x-auto border rounded-lg dark:border-gray-700"><table class="w-full text-center"><thead class="bg-gray-100 dark:bg-gray-700 font-bold"><tr><th class="p-2">Size</th><th class="p-2">Chiều cao</th><th class="p-2">Cân nặng</th><th class="p-2">Vòng bụng</th><th class="p-2">Vòng mông</th></tr></thead><tbody class="divide-y dark:divide-gray-600"><tr><td class="p-2 font-bold">S/28</td><td>160-165</td><td>50-54</td><td>68-72</td><td>84-88</td></tr><tr><td class="p-2 font-bold">M/29</td><td>160-165</td><td>55-61</td><td>72-76</td><td>88-92</td></tr><tr><td class="p-2 font-bold">L/30</td><td>166-172</td><td>62-68</td><td>76-80</td><td>92-95</td></tr><tr><td class="p-2 font-bold">XL/31</td><td>172-177</td><td>69-75</td><td>80-84</td><td>95-98</td></tr><tr><td class="p-2 font-bold">2XL/32</td><td>177-184</td><td>76-84</td><td>84-88</td><td>98-101</td></tr></tbody></table></div></div>
              </div>
              <div v-else-if="sizeTab === 'Nữ'" class="space-y-8 animate-fade-in">
                  <div><h4 class="font-bold text-lg mb-3 dark:text-white border-l-4 border-pink-500 pl-2">Áo Nữ</h4><div class="overflow-x-auto border rounded-lg dark:border-gray-700"><table class="w-full text-center"><thead class="bg-gray-100 dark:bg-gray-700 font-bold"><tr><th class="p-2">Size</th><th class="p-2">Chiều cao</th><th class="p-2">Cân nặng</th><th class="p-2">Rộng vai</th><th class="p-2">Vòng ngực</th></tr></thead><tbody class="divide-y dark:divide-gray-600"><tr><td class="p-2 font-bold">XS</td><td>145-150</td><td>35-39</td><td>35</td><td>74-77</td></tr><tr><td class="p-2 font-bold">S</td><td>150-155</td><td>40-45</td><td>36</td><td>78-82</td></tr><tr><td class="p-2 font-bold">M</td><td>156-160</td><td>46-51</td><td>37</td><td>83-87</td></tr><tr><td class="p-2 font-bold">L</td><td>161-164</td><td>52-57</td><td>38</td><td>87-91</td></tr></tbody></table></div></div>
                  <div><h4 class="font-bold text-lg mb-3 dark:text-white border-l-4 border-pink-500 pl-2">Quần Nữ</h4><div class="overflow-x-auto border rounded-lg dark:border-gray-700"><table class="w-full text-center"><thead class="bg-gray-100 dark:bg-gray-700 font-bold"><tr><th class="p-2">Size</th><th class="p-2">Chiều cao</th><th class="p-2">Cân nặng</th><th class="p-2">Vòng bụng</th><th class="p-2">Vòng mông</th></tr></thead><tbody class="divide-y dark:divide-gray-600"><tr><td class="p-2 font-bold">XS/25</td><td>145-150</td><td>35-39</td><td>52-62</td><td>82-86</td></tr><tr><td class="p-2 font-bold">S/26</td><td>150-155</td><td>40-45</td><td>62-66</td><td>86-90</td></tr><tr><td class="p-2 font-bold">M/27</td><td>156-160</td><td>46-51</td><td>66-70</td><td>90-94</td></tr><tr><td class="p-2 font-bold">L/28</td><td>161-164</td><td>52-57</td><td>70-74</td><td>94-98</td></tr></tbody></table></div></div>
              </div>
              <div v-else class="space-y-8 animate-fade-in">
                  <div><h4 class="font-bold text-lg mb-3 dark:text-white border-l-4 border-green-500 pl-2">Áo Trẻ Em</h4><div class="overflow-x-auto border rounded-lg dark:border-gray-700"><table class="w-full text-center"><thead class="bg-gray-100 dark:bg-gray-700 font-bold"><tr><th class="p-2">Size</th><th class="p-2">Chiều cao</th><th class="p-2">Cân nặng</th><th class="p-2">Vòng ngực</th></tr></thead><tbody class="divide-y dark:divide-gray-600"><tr><td class="p-2 font-bold">98 (2-3)</td><td>93-101</td><td>13-16</td><td>53-56</td></tr><tr><td class="p-2 font-bold">104 (4-5)</td><td>101-107</td><td>16-19</td><td>56-58</td></tr><tr><td class="p-2 font-bold">110 (6-7)</td><td>107-113</td><td>19-22</td><td>58-60</td></tr><tr><td class="p-2 font-bold">116 (8)</td><td>113-119</td><td>22-25</td><td>60-62</td></tr></tbody></table></div></div>
                  <div><h4 class="font-bold text-lg mb-3 dark:text-white border-l-4 border-green-500 pl-2">Quần Trẻ Em</h4><div class="overflow-x-auto border rounded-lg dark:border-gray-700"><table class="w-full text-center"><thead class="bg-gray-100 dark:bg-gray-700 font-bold"><tr><th class="p-2">Size</th><th class="p-2">Chiều cao</th><th class="p-2">Cân nặng</th><th class="p-2">Vòng bụng</th><th class="p-2">Vòng mông</th></tr></thead><tbody class="divide-y dark:divide-gray-600"><tr><td class="p-2 font-bold">104 (4-5)</td><td>101-107</td><td>16-19</td><td>48-50</td><td>57-60</td></tr><tr><td class="p-2 font-bold">110 (6-7)</td><td>107-113</td><td>19-22</td><td>50-52</td><td>60-63</td></tr><tr><td class="p-2 font-bold">116 (8)</td><td>113-119</td><td>22-25</td><td>52-54</td><td>63-66</td></tr></tbody></table></div></div>
              </div>
          </div>
      </div>
  </div>
  ...
  <div v-if="showTryOnGuide" class="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 backdrop-blur-sm" @click.self="showTryOnGuide = false">
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh]">
    
    <!-- Header -->
    <div class="flex justify-between items-center p-5 border-b dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
      <h3 class="text-lg font-bold text-gray-900 dark:text-white">Hướng dẫn chụp ảnh để thử đồ AI</h3>
      <button @click="showTryOnGuide = false" class="text-gray-500 hover:text-gray-800 dark:hover:text-white transition-colors">
    ...
    <div class="p-6 overflow-y-auto custom-scrollbar text-sm space-y-6 text-gray-700 dark:text-gray-300">
      <p class="font-medium text-base">Để AI xử lý chính xác và cho kết quả thử đồ đẹp nhất, vui lòng tuân thủ các yêu cầu sau:</p>

      <ul class="list-disc pl-5 space-y-3">
        <li>
          <strong>Định dạng ảnh:</strong> JPG, JPEG, PNG
        </li>
        <li>
          <strong>Dung lượng ảnh:</strong> Không quá <span class="font-semibold text-blue-600 dark:text-blue-400">3 MB</span>
        </li>
        <li>
          <strong>Độ phân giải ảnh:</strong> Nhỏ hơn <span class="font-semibold text-blue-600 dark:text-blue-400">4096 × 4096 px</span>
        </li>
        <li>
          ...
          <ul class="list-circle pl-5 mt-1 space-y-1 text-gray-600 dark:text-gray-400">
            ...
            <li>Hai tay phải được nhìn thấy rõ (không che khuất, không bắt chéo)</li>
            <li>Tránh các tư thế dang tay rộng, bắt chéo tay hoặc động tác quá phô trương</li>
          </ul>
        </li>
        <li>
          <strong>Loại trang phục phù hợp:</strong> Hoa văn và họa tiết <strong>đơn giản</strong>. Ví dụ: quần jean, áo polo, đồ tập yoga, váy, vest, áo thun, v.v.
        </li>
        <li>
          <strong>Ảnh quần áo (flat-lay):</strong> 
          <ul class="list-circle pl-5 mt-1 space-y-1 text-gray-600 dark:text-gray-400">
            <li>Ảnh <strong>trải phẳng</strong> (flat-lay), rõ nét và ngay ngắn</li>
...
<li>Trang phục phải <strong>chiếm phần lớn khung hình</strong></li>
          </ul>
        </li>
      </ul>
...
<div class="mt-6">
  <h4 class="font-semibold text-base mb-4 text-gray-800 dark:text-gray-200">Ảnh minh họa đúng / sai</h4>
  
  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    <!-- Ảnh ĐÚNG -->
    <div class="space-y-3">
      <div class="relative rounded-lg overflow-hidden border border-green-200 dark:border-green-800 shadow-sm bg-gray-50 dark:bg-gray-900 flex items-center justify-center min-h-[300px]">
        <img 
    ...
    <!-- Ảnh SAI -->
    <div class="space-y-3">
      <div class="relative rounded-lg overflow-hidden border border-red-200 dark:border-red-800 shadow-sm bg-gray-50 dark:bg-gray-900 flex items-center justify-center min-h-[300px]">
        <img 
  ...
  <p class="text-xs text-gray-500 dark:text-gray-400 mt-6 italic text-center">
    Ảnh ví dụ chỉ mang tính minh họa. Hãy chụp ảnh thật theo đúng hướng dẫn để có kết quả tốt nhất.
  </p>
</div>

  <div class="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-100 dark:border-blue-800">
    <p class="text-sm font-medium text-blue-800 dark:text-blue-300">
      Lưu ý: Ảnh không đáp ứng yêu cầu có thể dẫn đến kết quả thử đồ không chính xác hoặc thất bại.
    </p>
  </div>
    </div>
...
async function convertImageToPNG(imageUrl) {
    ...
    return new Promise((resolve, reject) => {
        img.onload = () => {
            ...
            }, 'image/png', 0.95); // 0.95 = chất lượng cao
        };
...
async function startTryOn() {
  ...
  try {
    ...
    const clothesUrl = getColorImage(tryOnSelectedColor.value.hex) || productImages.value[0];
...
const clothesFile = new File([clothesBlob], "clothes.png", { type: "image/png" });
// Nếu dùng JPEG: "clothes.jpg", { type: "image/jpeg" }
    ...
    while (pollAttempts.value < pollMaxAttempts.value) {
      ...
      if (resultData.error_code !== 0 || resultData.error_msg) {
        throw new Error(resultData.error_msg || "AI xử lý thất bại (mã lỗi từ server)");
      }
      ...
      console.log("⏳ Trạng thái task:", resultData.task_status, "- Chưa xong, chờ thêm...");
    }
...
watch(currentImageIndex, (newIndex) => {
    ...
    if (colorHex) {
        const matchingColor = colors.value.find(c => c.hex.toLowerCase() === colorHex);
...
function tryOnSelectColor(color) { tryOnSelectedColor.value = color; }
...
async function submitReview() {
    ...
    try {
        const payload = { user_id: userStore.profile?.id || 99, rating: newReview.rating, comment: newReview.comment, title: 'Đánh giá' };
...
function addToCart() {
  ...
  cart.addToCart({ id: product.value.id, name: product.value.name, price: product.value.price, image: selectedImage.value, 
    selectedColor: selectedColor.value?.name || selectedColor.value || null, selectedSize: selectedSize.value }, qty.value);
}
function toggleWishlist() {
  if (!product.value) return;
  wishlist.toggleWishlist({ id: product.value.id, name: product.value.name, price: product.value.price, image: selectedImage.value });
}
