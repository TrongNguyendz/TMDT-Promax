...
<template>
  <div class="group relative flex flex-col gap-2">
    ...
    <!-- 1. ẢNH SẢN PHẨM -->
    <div class="relative w-full aspect-[3/4] overflow-hidden rounded-2xl bg-gray-100 border border-gray-100 dark:border-gray-800 dark:bg-gray-900">
      
      <!-- Link chi tiết -->
      <RouterLink :to="`/product/${product.id}`" class="block h-full w-full">
        <img 
          :src="currentImage" 
          :alt="product.name" 
        ...
        <!-- BADGE HẾT HÀNG -->
        <div v-if="isOutOfStock" class="absolute inset-0 flex items-center justify-center bg-black/40 z-10">
          <span class="rotate-12 rounded bg-red-600 px-3 py-1 text-sm font-bold text-white shadow-lg border-2 border-white">
            HẾT HÀNG
          </span>
        </div>

        <!-- BADGE SALE -->
        <div v-else-if="product.discount_percent > 0" class="absolute top-2 left-2 rounded bg-red-600 px-2 py-1 text-xs font-bold text-white z-10">
      ...
      <button 
        ...
        :class="wishlist.isInWishlist(product.id) ? 'bg-red-50 text-red-500' : 'bg-white/90 text-gray-900 hover:bg-red-50 hover:text-red-500 dark:bg-gray-800/90 dark:text-gray-200 dark:hover:bg-red-500/20 dark:hover:text-red-400'"
        :aria-label="wishlist.isInWishlist(product.id) ? 'Bỏ yêu thích' : 'Thêm vào yêu thích'"
        title="Yêu thích"
      >
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" :fill="wishlist.isInWishlist(product.id) ? 'currentColor' : 'none'" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div class="flex flex-col px-1">
     ...
     <RouterLink 
      ...
      <div class="flex items-center gap-1.5 h-6 mb-1">
        <template v-if="colors.length > 0">
          <div 
            ...
            :title="color.name"
          >
            <div class="w-4 h-4 rounded-full border border-white/80 shadow-sm" :style="{ backgroundColor: color.hex }"></div>
          </div>
          <span v-if="remainingColors > 0" class="text-[10px] text-gray-500 dark:text-gray-400 ml-1">+{{ remainingColors }}</span>
        </template>
        <span v-else class="text-[10px] text-gray-500 dark:text-gray-400 italic">Tiêu chuẩn</span>
      </div>
      ...
      <!-- <RouterLink 
        :to="`/product/${product.id}`" 
        class="text-sm text-gray-800 dark:text-gray-200 hover:text-blue-600 dark:hover:text-blue-400 font-medium line-clamp-1 mt-0.5"
        :title="product.name"
      >
        {{ product.name }}
      </RouterLink> -->
...
const initImage = () => {
  ...
  if (productImages.value.length > 0) {
    ...
    currentImage.value = props.product.image || 'https://via.placeholder.com/400x533?text=No+Img';
  }
};
...
const buildImageMap = () => {
  ...
  if (props.product.images?.length) {
    const sorted = [...props.product.images].sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0));
...
const colors = computed(() => {
  ...
  attrs.forEach(attr => {
    if (attr.attribute_name?.toLowerCase() === 'màu') {
      ...
      if (match) {
        list.push({ name: match[1].trim(), hex: match[2].toLowerCase() });
      } else if (val.startsWith('#')) {
        list.push({ name: val, hex: val.toLowerCase() });
      } else {
        list.push({ name: val, hex: '#eeeeee' });
      }
    }
  });
...
const remainingColors = computed(() => {
  const total = props.product.attributes?.filter(a => a.attribute_name?.toLowerCase() === 'màu').length || 0;
...
const originalPrice = computed(() => {
  if (props.product.discount_percent > 0) {
    return Math.round(props.product.price / (1 - props.product.discount_percent / 100));
  }
